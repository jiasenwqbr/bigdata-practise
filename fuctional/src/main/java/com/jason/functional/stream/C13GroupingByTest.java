...
public class C13GroupingByTest {
    ...
    public static void main(String[] args) {
        ...
        */
//        Map<Integer, List<Integer>> result = stream.collect(Collectors.groupingBy(h -> h.name().length(),
...
//        Map<Integer, List<String>> result = stream.collect(groupingBy(h -> h.name().length(),
//                flatMapping(h -> h.name().chars().mapToObj(Character::toString), toList())
//        ));
//        System.out.println(result);
        /*
         ...
         */
//        Map<Integer, Long> collect = stream.collect(groupingBy(h -> h.name().length(),
//                counting()));
...
//        Map<Integer, Optional<Hero>> result = stream.collect(groupingBy(h -> h.name().length(),
//                minBy((a, b) -> a.strength() > b.strength() ? a.strength : b.strength())
//        ));
//        System.out.println(result);
//        Map<Integer, Optional<Hero>> result2 = stream.collect(groupingBy(h -> h.name().length(), maxBy(Comparator.comparingInt(Hero::strength))));
